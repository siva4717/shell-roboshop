...
curl -o /tmp/cart.zip https://roboshop-artifacts.s3.amazonaws.com/cart-v3.zip &>>$FILE_LOG
cd /app  &>>$FILE_LOG

rm -rf /app/*
VALIDATE $? "removing existing code"

unzip /tmp/cart.zip  &>>$FILE_LOG
VALIDATE $? "unzip"
...
systemctl enable cart &>>$FILE_LOG
VALIDATE $? "enable cart" 

systemctl start cart &>>$FILE_LOG
VALIDATE $? "start cart"
